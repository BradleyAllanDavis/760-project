...
if not os.path.exists('landmark_chunks'):
    os.makedirs('landmark_chunks')
...
def generate(): 
    ...
    def generate_chunk(img, label, i, name):
        ...
        for j,img_id in enumerate(in_memory_imgid):
            try:
                print(curr_index)
                loc = os.path.join("landmark/","{}_{}_{}".format(img_id,256,256))+".npy"
        ...
        print(curr_index)
        np.save("landmark_chunks/{}_img_{}.npy".format(name, i), img_resize[:curr_index])
        np.save("landmark_chunks/{}_label_{}.npy".format(name, i), out_label[:curr_index])
